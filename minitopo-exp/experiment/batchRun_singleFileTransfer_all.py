...
######  PROJECT List
PROJECT_LIST = ['mp-quic','quic-go','sa-ecf']
...
# fixed rtt
bandwidth_delay_list.append((1, 1))
for tmp in range(1, data_num):
     bandwidth_delay_list.append((tmp*step_bandwidth, 1))

## fixed bandwidth
bandwidth_delay_list.append((1, 1))
for tmp in range(1, data_num):
     bandwidth_delay_list.append((1, tmp*step_delay))

# low rtt low bandwidth to high rtt high bandwidth
bandwidth_delay_list.append((1, 1))
for tmp in range(1, data_num):
     bandwidth_delay_list.append((tmp*step_bandwidth, tmp*step_delay))
...
###### hllh path manager line
line_pathmanager_rtt_num = 162
line_pathmanager_rtt_num2 = 258
line_pathmanager_rtt_num3 = 325

line_pathmanager_bandwidth_num = 163
line_pathmanager_bandwidth_num2 = 259
line_pathmanager_bandwidth_num3 = 326

line_pathmanager_fixed_rtt_num = 166
line_pathmanager_fixed_rtt_num2 = 262
line_pathmanager_fixed_rtt_num3 = 329

line_pathmanager_fixed_bandwidth_num = 167
line_pathmanager_fixed_bandwidth_num2 = 263
line_pathmanager_fixed_bandwidth_num3 = 330
...
###### transfer one file or two simultaneously
single_file = '0'
        ...
        for (bandwidth, delay) in bandwidth_delay_list:
            ...
            for line in lines:
                if counter == line_if_browse_num:
                    ...
                    fw.write(
                        '    mptcpTopos = [{\'netem\': [(0, 0, \'loss 0.00%%\'), (1, 0, \'loss 0.00%%\')], \'paths\': [{\'queuingDelay\': \'0\', \'delay\': \'%f\'' % (
                                float(delay) / 2.0) + ', \'bandwidth\': \'%d\'' % bandwidth + '}, {\'queuingDelay\': \'0\', \'delay\': \'%f\'' % (
                                float(fixed_rtt) / 2.0) + ', \'bandwidth\': \'%d\'}]}]\n' % fixed_bandwidth)
                else:
                    fw.write(line)
                counter = counter+1
            fr.close()
