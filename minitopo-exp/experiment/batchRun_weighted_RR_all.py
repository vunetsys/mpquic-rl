...
######  PROJECT List
PROJECT_LIST = ['quic-go','sa-ecf'] #'mp-quic','quic-go','sa-ecf'
        ...
        for scheduler in SCHEDULER_LIST:
            ...
            for line in lines:
                if counter == line_if_browse_num:
                    ...
                    fw.write(
                        '                    SINGLE_FILE: \"%s\", # default 0, means transfer two file simultaneously, if 1 only transfer the first file\n' % single_file)
                elif counter == line_browse_PROJECT_num:
                    fw.write(
                        '                    PROJECT: \"%s\",     # quic-go is the prioritized stream scheduling project, mp-quic is the original multipath-quic project\n' % project)
                elif counter == line_website_num:
                    fw.write('                    JSON_FILE: \"%s\",   # specify websites to download\n' % website)
                elif counter == line_fstream_scheduler:
                    fw.write(
                        '                    PATH_SCHEDULER:\"%s\",   # quic-go param: MultiPath; SinglePath\n' % scheduler)
            ...
            ###############################  SCP  ############################
            os.system(
                'scp -P 3022 ' + pathmanager_sourcefile + ' mininet@127.0.0.1:/home/mininet/go/src/github.com/lucas-clemente/quic-go')
            repeat_num = 5
            while (repeat_num > 0):
...
######  PROJECT List
PROJECT_LIST = ['quic-go','sa-ecf'] #'quic-go','sa-ecf'
