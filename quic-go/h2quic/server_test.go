...
func (s *mockSession) GetOrOpenStreamPriority(id protocol.StreamID, priority *protocol.Priority) (quic.Stream, error) {
	return s.dataStream, nil
}
func (s *mockSession) GetOrOpenStreamPrioritySize(id protocol.StreamID, priority *protocol.Priority) (quic.Stream, error) {
	return s.dataStream, nil
}
...
func (s *mockSession) OpenStreamPrioritySync(priority *protocol.Priority) (quic.Stream, error) {
	if s.blockOpenStreamSync {
		time.Sleep(time.Hour)
	}
	return s.OpenStream()
}
func (s *mockSession) OpenStreamPrioritySizeSync(priority *protocol.Priority) (quic.Stream, error) {
	if s.blockOpenStreamSync {
		time.Sleep(time.Hour)
	}
	return s.OpenStream()
...
func (s *mockSession) SetStreamPriority(protocol.StreamID, *protocol.Priority) error {
	panic("not implemented")
}
...
var _ = Describe("H2 server", func() {
	...
	Context("handling requests", func() {
		...
		It("Cancels the request context when the datastream is closed", func() {
			var handlerCalled bool
			s.Handler = http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				defer GinkgoRecover()
				err := r.Context().Err()
	...
	It("errors when listening fails", func() {
		...
		quicConfig := &quic.Config{
			PathScheduler: "SinglePath",
		}
		err := ListenAndServeQUIC("", fullpem, privkey, nil, quicConfig)
