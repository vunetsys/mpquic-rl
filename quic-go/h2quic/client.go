...
type client struct {
	...
	//SHI: urls and their corresponding priority
...
//SHI: Client contain stream priority info
func newClientPriority(
	hostname string,
	tlsConfig *tls.Config,
	opts *roundTripperOpts,
	quicConfig *quic.Config,
...
func (c *client) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if utils.Debug() {
		utils.Debugf("in h2quic.RoundTrip(): after NewRequest, req.URL = %s", req.URL.String())
	}
	...
	// (priority set in terminal, and passed by the client.quicConfig)

	if len(c.priorityURL) == 0 {
		return nil, errors.New("quic http2: priority nil")
	}
	priority, ok := c.priorityURL[req.URL.String()]
	if !ok {
		utils.Infof("%s miss priority\n", req.URL.String())
		return nil, errors.New("quic http2: miss priority")
	}

	if utils.Debug() {
		utils.Debugf("url: %s, weight: %d, dependency %d \n", req.URL.String(), priority.Weight, priority.StreamDep)
	}

	priorityTran := new(protocol.Priority)

	priorityTran.Weight = priority.Weight
	priorityTran.Dependency = protocol.StreamID(priority.StreamDep)
	priorityTran.Exclusive = priority.Exclusive

	dataStream, err := c.session.OpenStreamPrioritySizeSync(priorityTran)
	...
	//SHI:  write request with priority, size
	if utils.Debug() {
		utils.Debugf("Client.go: SHI:  write request with weight %d, size\n", priority.Weight)
	}
	err = c.requestWriter.WritePrioritySizeRequest(req, dataStream.StreamID(), priority, endStream, requestedGzip)
