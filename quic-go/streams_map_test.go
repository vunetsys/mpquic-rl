...
var _ = Describe("Streams Map", func() {
	...
	// SHI: extend streamsMap function
	setNewStreamsMapPriority := func(p protocol.Perspective) {
		mockCpm = mocks.NewMockConnectionParametersManager(mockCtrl)

		mockCpm.EXPECT().GetMaxOutgoingStreams().AnyTimes().Return(uint32(maxOutgoingStreams))
		mockCpm.EXPECT().GetMaxIncomingStreams().AnyTimes().Return(uint32(maxIncomingStreams))

		m = newStreamsMapPrioritySize(nil, p, mockCpm)
		m.newStreamPrioritySize = func(id protocol.StreamID, priority *protocol.Priority) *stream {
			return newStreamPrioritySize(id, priority, nil, nil, nil)
		}
	}
	...
	Context("getting and creating priority streams", func() {
		Context("as a server", func() {

			BeforeEach(func() {
				setNewStreamsMapPriority(protocol.PerspectiveServer)
			})
			...
			It("gets new streams", func() {
				...
				//Expect(s.Priority()).To(Equal(uint8(50)))
	...
	Context("getting and creating streams", func() {
		...
		Context("as a client", func() {
			BeforeEach(func() {
				setNewStreamsMapPriority(protocol.PerspectiveClient)
			})
