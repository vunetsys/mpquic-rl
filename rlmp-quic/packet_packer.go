...
/*
// SHI: ack and stopWaiting
func (p *packetPacker) PackACKPacketOfPath(pth *path) (*packedPacket, error) {
	if p.streamFramer.HasCryptoStreamFrame() {
		return p.packCryptoPacket(pth)
	}
	encLevel, sealer := p.cryptoSetup.GetSealer()
	...
	if isPing {
		...
		maxSize := protocol.MaxPacketSize - protocol.ByteCount(sealer.Overhead()) - publicHeaderLength
		payloadFrames, err = p.composeNextACKPacket(maxSize, p.canSendData(encLevel), pth)
		if err != nil {
			return nil, err
		}
	}
...
// SHI: PackPacket packs data of streams reside in this path
...
// SHI: PackPacket packs a new packet of a stream
...
/*
//SHI: compose without stream frame
func (p *packetPacker) composeNextACKPacket(
	maxFrameSize protocol.ByteCount,
	canSendStreamFrames bool,
	pth *path,
) ([]wire.Frame, error) {
