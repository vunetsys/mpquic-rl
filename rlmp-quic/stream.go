...
type stream struct {
	...
	//SHI: assigned pathID
	pathVolume map[protocol.PathID]float64
...
func newStream(StreamID protocol.StreamID,
	...
	s := &stream{
		...
		size:               protocol.ByteCount(0), //default 0
		checksize:          false,                 //default false
		pathVolume:         make(map[protocol.PathID]float64),
...
func newStreamPriority(StreamID protocol.StreamID,
	priority *protocol.Priority,
	onData func(),
	onReset func(protocol.StreamID, protocol.ByteCount),
	flowControlManager flowcontrol.FlowControlManager) *stream {
...
func newStreamPrioritySize(StreamID protocol.StreamID,
	priority *protocol.Priority,
	onData func(),
	onReset func(protocol.StreamID, protocol.ByteCount),
	flowControlManager flowcontrol.FlowControlManager) *stream {
	...
	//fmt.Printf("stream.newStreamPrioritySize(): weight %d\n", priority.Weight)

	s.ctx, s.ctxCancel = context.WithCancel(context.Background())
	// if utils.Debug() {
	// 	utils.Debugf("SHI: newStreamPriority (called by newStreamsMap), stream %d, priority %d, size %d\n", s.streamID, s.priority.Weight, s.size)
	// }
	return s
}
...
func (s *stream) Size() protocol.ByteCount {
	return s.size
}
