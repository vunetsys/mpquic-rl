...
var _ = Describe("Client", func() {
	var (
		client, clientPriority *client
		priorityURL            map[string]*http2.PriorityParam
		//sizeURL                map[string]uint32
		session      *mockSession
	...
	BeforeEach(func() {
		...
		priorityURL["https://quic.clemente.io:1337/foobar.html"] = &priority
		priorityURL["https://quic.clemente.io:1337/file1.dat"] = &priority
		priorityURL["https://quic.clemente.io:1337/pages/360.cn/360.cn/index.html"] = &priority
		clientPriority = newClientPriority(hostname, nil, &roundTripperOpts{}, nil, priorityURL)
		Expect(client.hostname).To(Equal(hostname))
		Expect(clientPriority.hostname).To(Equal(hostname))

		session = &mockSession{}
		session.ctx, session.ctxCancel = context.WithCancel(context.Background())
		client.session = session
		clientPriority.session = session
		headerStream = newMockStream(3)
		client.headerStream = headerStream
		clientPriority.headerStream = headerStream

		client.requestWriter = newRequestWriter(headerStream)
		clientPriority.requestWriter = newRequestWriter(headerStream)
	...
	It("dials with priority stream", func(done Done) {
		priority := protocol.Priority{Dependency: 0, Weight: 50}
		var dataStream *mockStream = newMockStreamPriority(5, &priority)

		client = newClient("localhost:1337", nil, &roundTripperOpts{}, nil)
		session.streamsToOpen = []quic.Stream{newMockStreamPriority(3, &priority), dataStream}
		dialAddr = func(hostname string, _ *tls.Config, _ *quic.Config) (quic.Session, error) {
			return session, nil
		}
		close(headerStream.unblockRead)
		...
		Eventually(func() quic.Session { return client.session }).Should(Equal(session))
		Eventually(dataStream.priority.Weight).Should(Equal(uint8(50)))
		close(done)
	}, 2)
	...
	Context("Doing requests", func() {
		...
		It("does a request", func(done Done) {
			...
			var doReturned bool
			go func() {
				doRsp, doErr = clientPriority.RoundTrip(request)
				doReturned = true
			}()

			Eventually(func() []byte { return headerStream.dataWritten.Bytes() }).ShouldNot(BeEmpty())
			Eventually(func() map[protocol.StreamID]chan *http.Response { return clientPriority.responses }).Should(HaveKey(protocol.StreamID(5)))
			rsp := &http.Response{
				Status:     "418 I'm a teapot",
				StatusCode: 418,
			}
			Expect(clientPriority.responses[5]).ToNot(BeClosed())
			Expect(clientPriority.headerErrored).ToNot(BeClosed())
			clientPriority.responses[5] <- rsp
		...
		It("closes the quic client when encountering an error on the header stream", func(done Done) {
			...
			go func() {
				defer GinkgoRecover()
				var err error
				rsp, err := clientPriority.RoundTrip(request)
				Expect(err).To(MatchError(clientPriority.headerErr))
				Expect(rsp).To(BeNil())
				doReturned = true
			}()

			Eventually(func() bool { return doReturned }).Should(BeTrue())
			Expect(clientPriority.headerErr).To(MatchError(qerr.Error(qerr.HeadersStreamDataDecompressFailure, "cannot read frame")))
			Expect(clientPriority.session.(*mockSession).closedWithError).To(MatchError(clientPriority.headerErr))
			close(done)
		}, 2)
		...
		It("returns subsequent request if there was an error on the header stream before", func(done Done) {
			...
			go func() {
				defer GinkgoRecover()
				_, err := clientPriority.RoundTrip(request)
		...
		It("blocks if no stream is available", func() {
			...
			go clientPriority.handleHeaderStream()
		...
		Context("validating the address", func() {
			...
			FIt("find http priority", func() {
				req, err := http.NewRequest("https", "https://quic.clemente.io:1337/pages/360.cn/360.cn/index.html", nil)
				Expect(err).ToNot(HaveOccurred())
				_, err = clientPriority.RoundTrip(req)
			})
			It("find http priority", func() {
				req, err := http.NewRequest("https", "https://quic.clemente.io:1337/foobar.html", nil)
				Expect(err).ToNot(HaveOccurred())
				_, err = clientPriority.RoundTrip(req)
			})
