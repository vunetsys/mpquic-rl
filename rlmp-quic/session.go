...
type session struct {
	...
	paths     map[protocol.PathID]*path //will not delete record when a path is closed
	openPaths []protocol.PathID         //SHI: needed for round-robin path sending, will not delete record when a path is closed
...
func newSession(
	...
	s := &session{
		...
		openPaths:    make([]protocol.PathID, 0),
		closedPaths:  make(map[protocol.PathID]bool),
		streamToPath: make(map[protocol.StreamID][]protocol.PathID),
...
func (s *session) setup(
	...
	if pconnMgr == nil && conn != nil {
		...
		s.openPaths = append(s.openPaths, protocol.InitialPathID)
	...
	//SHI:modify to  use priority stream
	//s.streamsMap = newStreamsMap(s.newStream, s.perspective, s.connectionParameters)
	//s.streamsMap = newStreamsMapPriority(s.newStreamPriority, s.perspective, s.connectionParameters)
	s.streamTree = newStreamTree()
	// if utils.Debug() {
	// 	utils.Debugf("session.go  Line 250 runloop initiate streamsMap\n")
	// }
	s.streamsMap = newStreamsMapTree(s.newStreamPrioritySize, s.perspective, s.connectionParameters, s.streamTree)
	s.streamFramer = newStreamFramerTree(s.streamsMap, s.flowControlManager, s.streamTree)
	// if utils.Debug() {
	// 	utils.Debugf("session.go  Line 255 runloop initiate streamsMap\n")
	// }
	s.pathTimers = make(chan *path)
	if s.streamTree != nil {
		s.streamTree.streamFramer = s.streamFramer
	}

	var err error
	if s.perspective == protocol.PerspectiveServer {
		// if utils.Debug() {
		// 	utils.Debugf("session.go  Line 265 runloop GetOrOpen cryptoStream\n")
		// }
		cryptoStream, _ := s.GetOrOpenStream(1)
		_, _ = s.AcceptStream() // don't expose the crypto stream
		// if utils.Debug() {
		// 	utils.Debugf("session.go  Line 270 \n")
		// }
		...
		if s.version.UsesTLS() {
			// if utils.Debug() {
			// 	utils.Debugf("session.go  Line 278 \n")
			// }
			...
			// if utils.Debug() {
			// 	utils.Debugf("session.go  Line 290 \n")
			// }
		} else {
			// if utils.Debug() {
			// 	utils.Debugf("session.go  Line 295 \n")
			// }
			...
			// if utils.Debug() {
			// 	utils.Debugf("session.go  Line 309 \n")
			// }
		}
	} else {
		// if utils.Debug() {
		// 	utils.Debugf("session.go  Line 296 runloop OpenStream cryptoStream\n")
		// }
		cryptoStream, _ := s.OpenStream()
		if s.version.UsesTLS() {

			// if utils.Debug() {
			// 	utils.Debugf("session.go  Line 302 \n")
			// }
			s.cryptoSetup, err = handshake.NewCryptoSetupTLS(
				hostname,
				s.perspective,
			...
			// if utils.Debug() {
			// 	utils.Debugf("session.go  Line 313 \n")
			// }
		} else {
			// if utils.Debug() {
			// 	utils.Debugf("session.go  Line 317 \n")
			// }
			...
			// if utils.Debug() {
			// 	utils.Debugf("session.go  Line 331 \n")
			// }
		}
	}
	if err != nil {
	...
	// if utils.Debug() {
	// 	utils.Debugf("session.go  Line 327 runloop after handshake\n")
	// }
...
func (s *session) run() error {
	...
	go func() {
		// if utils.Debug() {
		// 	utils.Debugf("session.go  Line 377 HandleCryptoStream in perspective %x\n", s.perspective)
		// }
		if err := s.cryptoSetup.HandleCryptoStream(); err != nil {
			// if utils.Debug() {
			// 	utils.Debugf("session.go  Line 381 error closing session  in perspective %x\n", s.perspective)
			// }
			s.Close(err)
		}
	}()
...
//SHI: add localPconn net.PacketConn for convenience of path creation
func (s *session) handleFramesNew(fs []wire.Frame, p *path, localPconn net.PacketConn) error {
	for _, ff := range fs {
		var err error
		...
		case *wire.PathsFrame:
			// So far, do nothing, no actual use of s.remoteRTTs
			s.pathsLock.RLock()
			...
			// SHI: server check if there are new paths to create
			if s.perspective == protocol.PerspectiveServer {
				s.pathManager.createPathsFromRemotePathsFrame(frame, localPconn)
			}
			s.pathsLock.RUnlock()
		default:
...
//SHI: check if we have RTT statistics of all paths
func (s *session) knowAllPathRTT() bool {
	s.pathsLock.RLock()
...
//SHI: check if we have Bandwidth statistics of all paths
// func (s *session) knowAllPathBandwidth() bool {
// 	s.pathsLock.RLock()
// 	defer s.pathsLock.RUnlock()

// 	for _, pth := range s.paths {
// 		if pth.bandwidthStats == 0 {
// 			return false
// 		}
// 	}
// 	return true
...
func (s *session) closePath(pthID protocol.PathID, sendClosePathFrame bool) error {
	...
	for _, streamID := range s.paths[pthID].streamIDs {
		//for each stream in this path
		//delete streamToPath record
		err := s.streamToPath.DeleteOne(streamID, pthID)
		if err != nil {
			return err
		}

		//TODO: delete record of stream

		find := false
		for pid := range s.streamsMap.streams[streamID].pathVolume {
			if pid == pthID {
				delete(s.streamsMap.streams[streamID].pathVolume, pid)
				find = true
				break
			}
		...
		if !find {
			return errors.New("nothing to delete in s.streamsMap.streams[streamID].pathVolume")
		}

		//TODO: if stream scheduled on only this path, maybe turn streams on this path to initial path
		// if len(pathsOfStream) == 0 {

	}
	delete(s.scheduler.numstreams, pthID)
...
func (s *session) sendPackedPacketOfStream(packet *packedPacket, pth *path, id protocol.StreamID) error {
	defer putPacketBuffer(packet.raw)
	err := pth.sentPacketHandler.SentPacket(&ackhandler.Packet{
		PacketNumber:    packet.number,
		Frames:          packet.frames,
	...
	s.logPacketOfStream(packet, pth.pathID, id)
	return pth.conn.Write(packet.raw)
}
...
//SHI: send ping for all paths on a low-rtt path
//(if each path is responsible for sending its own ping, slow paths will be recognized by the peer at a large delay)
// func (s *session) sendPings() error {
...
func (s *session) logPacket(packet *packedPacket, pathID protocol.PathID) {
	...
	utils.Debugf("-> %s, Sending packet 0x%x (%d bytes) for connection %x on path %x, %s", time.Now(), packet.number, len(packet.raw), s.connectionID, pathID, packet.encryptionLevel)
	for _, frame := range packet.frames {
		wire.LogFrame(frame, true)
	}
}

func (s *session) logPacketOfStream(packet *packedPacket, pathID protocol.PathID, sid protocol.StreamID) {
	if !utils.Debug() {
		// We don't need to allocate the slices for calling the format functions
		return
	}
	utils.Debugf("-> %s, Sending packet 0x%x (%d bytes) of Stream %d for connection %x on path %x, %s", time.Now(), packet.number, len(packet.raw), sid, s.connectionID, pathID, packet.encryptionLevel)
	for _, frame := range packet.frames {
		wire.LogFrame(frame, true)
	}
}
...
// SHI:
func (s *session) GetOrOpenStreamPriority(id protocol.StreamID, priority *protocol.Priority) (Stream, error) {
	str, err := s.streamsMap.GetOrOpenStreamPriority(id, priority)
	if str != nil {
...
// SHI:
func (s *session) GetOrOpenStreamPrioritySize(id protocol.StreamID, priority *protocol.Priority) (Stream, error) {
...
func (s *session) OpenStreamPrioritySync(priority *protocol.Priority) (Stream, error) {
	return s.streamsMap.OpenStreamPrioritySync(priority)
}

func (s *session) OpenStreamPrioritySizeSync(priority *protocol.Priority) (Stream, error) {
	return s.streamsMap.OpenStreamPrioritySizeSync(priority)
}

func (s *session) SetStreamPriority(id protocol.StreamID, priority *protocol.Priority) error {
	if s.streamTree == nil {
		return nil
	}
	err := s.streamTree.maybeSetWeight(id, priority.Weight)
	if err != nil {
		return err
	}
	err = s.streamTree.maybeSetParent(id, priority.Dependency, priority.Exclusive)
	if err != nil {
		return err
	}
...
func (s *session) newStreamPriority(id protocol.StreamID, priority *protocol.Priority) *stream {
	// TODO: find a better solution for determining which streams contribute to connection level flow control
	if id == 1 || id == 3 {
		s.flowControlManager.NewStream(id, false)
	} else {
	...
	return newStreamPriority(id, priority, s.scheduleSending, s.queueResetStreamFrame, s.flowControlManager)
}

func (s *session) newStreamPrioritySize(id protocol.StreamID, priority *protocol.Priority) *stream {
	//fmt.Printf("session.newStreamPrioritySize(): weight %d\n", priority.Weight)
	...
	return newStreamPrioritySize(id, priority, s.scheduleSending, s.queueResetStreamFrame, s.flowControlManager)
}

// garbageCollectStreams goes through all streams and removes EOF'ed streams
// from the streams map.
// SHI: only server call this function
func (s *session) garbageCollectStreams() {
	s.streamsMap.Iterate(func(str *stream) (bool, error) {
		...
		if str.finished() {
			...
			//SHI: delete records about this stream assigned to path
			pthIDs := s.streamToPath[id]
			s.streamToPath.Delete(id)
			if utils.Debug() {
				utils.Debugf("garbageCollectStreams() delete stream %d", id)
			}

			for j := 0; j >= 0 && j < len(pthIDs); j++ {
				pthID := pthIDs[j]
				// delete record in pth.streamIDs
				for i := 0; i >= 0 && i < len(s.paths[pthID].streamIDs); i++ {
					tmp := s.paths[pthID].streamIDs[i]
					if tmp == id {
						s.paths[pthID].streamIDs = append(s.paths[pthID].streamIDs[:i], s.paths[pthID].streamIDs[i+1:]...)
						break
					}
				}
				//delete record in pth.streamOrder
				for i := 0; i >= 0 && i < len(s.paths[pthID].streamOrder); i++ {
					tmp := s.paths[pthID].streamOrder[i]
					if tmp == id {
						s.paths[pthID].streamOrder = append(s.paths[pthID].streamOrder[:i], s.paths[pthID].streamOrder[i+1:]...)
						break
					}
				}
				//delete record in pth.streamQuota
				delete(s.paths[pthID].streamQuota, id)

				s.scheduler.numstreams[pthID]--
			}
			if err != nil {
				return false, err
			}
