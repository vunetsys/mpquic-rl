...
type mockPath struct {
	...
	streamIDs []protocol.StreamID

	potentiallyFailed utils.AtomicBool
	rttStats          *congestion.RTTStats
}

func (p *mockPath) SendingAllowed() bool {
	return true
}
...
var _ = Describe("Session", func() {
	...
	// TODO: SHI: multipath test and debug
	Context("scheduling paths", func() {
		//unpassed test but doesn't affect any function
		PIt("schedule stream to path, transmit ack of unused path", func() {
			sess.packer.cryptoSetup = &mockCryptoSetup{encLevelSeal: protocol.EncryptionForwardSecure}

			//perspective client path ID
			pthA := &path{pathID: 1,
				sess: sess,
			}
			pthA.setup(nil)
			...
			pthA.rttStats.UpdateRTT((300 * time.Millisecond), (100 * time.Millisecond), time.Time{})
			Expect(pthA.rttStats.SmoothedRTT()).To(Equal((200 * time.Millisecond)))

			// pthB.rttStats.UpdateRTT((400 * time.Millisecond), (100 * time.Millisecond), time.Time{})
			// Expect(pthB.rttStats.SmoothedRTT()).To(Equal((300 * time.Millisecond)))

			// pthC.rttStats.UpdateRTT((600 * time.Millisecond), (100 * time.Millisecond), time.Time{})
			// Expect(pthC.rttStats.SmoothedRTT()).To(Equal((500 * time.Millisecond)))

			sess.paths[pthA.pathID] = pthA
			sess.openPaths = append(sess.openPaths, pthA.pathID)

			// sess.paths[pthB.pathID] = pthB
			// sess.paths[pthC.pathID] = pthC

			//sess.streamsMap = newStreamsMapPriority(sess.newStreamPriority, protocol.PerspectiveClient, sess.connectionParameters)

			//sess.streamFramer = newStreamFramer(sess.streamsMap, sess.flowControlManager)

			priority1 := &protocol.Priority{Weight: 50, Dependency: 0, Exclusive: false}
			priority2 := &protocol.Priority{Weight: 30, Dependency: 0, Exclusive: false}
			priority3 := &protocol.Priority{Weight: 20, Dependency: 0, Exclusive: false}

			s1, err := sess.GetOrOpenStreamPriority(5, priority1)
			Expect(err).NotTo(HaveOccurred())
			s2, err := sess.GetOrOpenStreamPriority(7, priority2)
			Expect(err).NotTo(HaveOccurred())

			s3, err := sess.GetOrOpenStreamPriority(9, priority3)
			Expect(err).NotTo(HaveOccurred())

			// Put data directly into the streams
			s1.(*stream).dataForWriting = []byte("foobar1")
			s2.(*stream).dataForWriting = []byte("foobar2")
			s3.(*stream).dataForWriting = []byte("foobar3")

			//add ack for unused path
			//packetNumber := protocol.PacketNumber(0x035E)
			//sess.paths[5].receivedPacketHandler.ReceivedPacket(packetNumber, true)
	...
	Context("retransmissions", func() {
		...
		Context("for packets after the handshake", func() {
			It("sends a StreamFrame from a packet queued for retransmission", func() {
				_, erro := sess.GetOrOpenStream(5) // SHI: before retransmit data of this stream must first open it
	...
	//SHI: any data needs to be retransmit should first has a stream ID
	...
	Context("scheduling sending", func() {
		...
		Context("bundling of small packets", func() {
			...
			// SHI: adapted to separate packing and sending
			It("bundles two small frames of different streams into separate packets and send", func() {
				s1, err := sess.GetOrOpenStream(5)
				Expect(err).NotTo(HaveOccurred())
				s2, err := sess.GetOrOpenStream(7)
				s3, err := sess.GetOrOpenStream(9)
				...
				//packet := <-mconn.written
				//Expect(packet).To(ContainSubstring("foobar1"))
				//Expect(packet).To(ContainSubstring("foobar3"))
			})
			...
			// SHI : prioritize packing of stream1 and 3;
			//	packet1:ack frames are packed into stream1's data,
			//	packet2:foobar1,
			//	packet3:foobar2
	...
	Context("getting streams", func() {
		...
		// SHI
		It("returns a new priority stream", func() {
			priority := protocol.Priority{Weight: 50, Dependency: 0, Exclusive: false}
			str, err := sess.GetOrOpenStreamPriority(11, &priority)
			Expect(err).ToNot(HaveOccurred())
			Expect(str).ToNot(BeNil())
			Expect(str.StreamID()).To(Equal(protocol.StreamID(11)))
			Expect(str.Priority().Weight).To(Equal(uint8(50)))
			Expect(str.Priority().Dependency).To(Equal(protocol.StreamID(0)))
			Expect(str.Priority().Exclusive).To(Equal(false))
		...
		// SHI:
		It("opens priority streams synchronously", func() {
			priority := protocol.Priority{Weight: 50, Dependency: 0, Exclusive: false}

			str, err := sess.OpenStreamPrioritySync(&priority)
			Expect(err).ToNot(HaveOccurred())
			Expect(str).ToNot(BeNil())
			Expect(str.Priority().Weight).To(Equal(priority.Weight))
		})
	})
