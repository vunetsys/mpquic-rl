...
func (f *flowControlManager) UpdateHighestReceived(streamID protocol.StreamID, byteOffset protocol.ByteCount) error {
	...
	increment, errReorder := streamFlowController.UpdateHighestReceived(byteOffset) //SHI
	if errReorder != nil {                                                          //SHI

		f.numOutofOrder = f.numOutofOrder + 1

		if utils.Debug() {
			utils.Debugf(" error out of order happened \n")
			utils.Debugf(" out of order count: = %d\n", f.numOutofOrder)
